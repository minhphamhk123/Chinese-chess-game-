...
bool Player::XuatCacNuocCoTheDi(int Type, Diem*& a, int n)
{
	...
	cout << "|               |        An quan       |" << endl;
	GoTo(30, 28);
	cout << "| Di chuyen toi |----------------------|" << endl;
	GoTo(30, 29);
	cout << "|               | Ten quan |  Toa do   |" << endl;
	GoTo(30, 30);
	cout << "|---------------+----------+-----------|" << endl;
	...
	for (int i = 0; i < n; i++)
	{
		...
		else
		{
			...
			else if (KiemTraTrangThaiToaDo(a[i]) == 1 && a[i].GetX() != -1 && a[i].GetY() != -1)
			{
				...
				int k;
				for (int h = 0; h < MAX; h++)
					if (Trang[h]->GetDiemDangDung() == a[i])
						k = h;
				k = Trang[k]->GetIndex();
				if (k == 1)
					cout << "Tot";
	...
	GoTo(30, (DiChuyen > AnQuan) ? DiChuyen : AnQuan);
	cout << "+--------------------------------------+" << endl;
	return flag;
}
...
bool Player::Go(int Type, int Index)
{
	...
	if (TrongSoDauTienCuaQuanCo != -1)
	{
		...
		cout << "+----------+---------+-----------------------------------------------+" << endl;
		cout << "| Ten quan | Toa do  |                    Cach di                    |" << endl;
		cout << "+----------+---------+-----------------------------------------------+" << endl;
		if (Index == 1)
			cout << "|   Tot    |         |                                               |" << endl;
		if (Index == 2)
...
void Player::StartGame()
{
	...
	while (true)
	{
		...
		cout << "---------------------------\n";
		GoTo(21, 1);
		cout << "-----DanhSachCacQuanCo-----" << endl;
		GoTo(21, 2);
		cout << "   1.TOT   -  S(Soldier)" << endl;
		GoTo(21, 3);
		cout << "   2.PHAO  -  C(Cannon)" << endl;
		GoTo(21, 4);
		cout << "   3.XE    -  R(Rook)" << endl;
		GoTo(21, 5);
		cout << "   4.NGUA  -  H(Horse)" << endl;
		GoTo(21, 6);
		cout << "   5.TUONG -  E(Elephant)" << endl;
		GoTo(21, 7);
		cout << "   6.SI    -  A(Advisor)" << endl;
		GoTo(21, 8);
		cout << "   7.TUONG -  K(King)" << endl;
		GoTo(21, 9);
		cout << "0.Ket thuc game" << endl;
		GoTo(21, 10);
		cout << "---------------------------\n";
		GoTo(21, 11);
		if (Type == 1)
		...
		GoTo(0, 13);
		int luachon;
		cout << "Chon mot quan co (Tu 1 den 7): ";
		cin >> luachon;
